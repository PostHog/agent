...
export class Agent {
    ...
    async runTask(taskOrId: Task | string, options: import('./types.js').TaskExecutionOptions = {}): Promise<void> {
        ...
        // Determine working directory: worktree (for concurrent tasks) or branch switching (for single tasks)
        let cwd: string;
        let worktreePath: string | null = null;

        if (useWorktree) {
            worktreePath = await this.prepareTaskWorktree(taskSlug);
            cwd = worktreePath;
        } else {
        ...
        try {
            ...
            if (useWorktree && worktreePath) {
                try {
                    this.logger.info('Cleaning up worktree', { worktreePath });
                    await this.gitManager.removeWorktree(worktreePath, true);
                    this.emitEvent(this.adapter.createStatusEvent('worktree_removed', { path: worktreePath }));
                } catch (cleanupError) {
                    this.logger.error('Failed to cleanup worktree', { worktreePath, error: cleanupError });
                    // Don't throw - worktree cleanup failure shouldn't mask the original error
                }
            }
...
Generated by PostHog Agent`;
    ...
    /**
     * Prepares a git worktree for task execution, enabling concurrent tasks.
     * Returns the worktree path to use as the working directory.
     */
    private async prepareTaskWorktree(taskSlug: string): Promise<string> {
        const worktreePath = this.gitManager.getWorktreePath(taskSlug);
        const branchName = `posthog/task-${taskSlug}`;

        this.logger.info('Preparing task worktree', { taskSlug, worktreePath, branchName });

        // Check if worktree already exists
        const worktreeExists = await this.gitManager.worktreeExists(worktreePath);
        if (worktreeExists) {
            this.logger.info('Reusing existing worktree', { worktreePath });
            return worktreePath;
        }

        // Ensure main repo is clean before creating worktree
        if (await this.gitManager.hasChanges()) {
            throw new Error('Cannot start task with uncommitted changes in main repository. Please commit or stash your changes first.');
        }

        // Create the worktree with the task branch
        await this.gitManager.createWorktree(branchName, worktreePath);
        this.emitEvent(this.adapter.createStatusEvent('worktree_created', {
            branch: branchName,
            path: worktreePath
        }));

        this.logger.info('Worktree created successfully', { taskSlug, worktreePath });
        return worktreePath;
    }

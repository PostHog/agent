...
import { ensureTimestamp } from './utils/notification-helpers.js';
...
Generated by PostHog Agent`;
    ...
    sendNotification(notification: import('./types.js').AgentNotification): void {
        // Ensure notification has timestamp in _meta
        const timestampedNotification = ensureTimestamp(notification);

        // Log if debug is enabled
        if (this.debug) {
            console.log(JSON.stringify(timestampedNotification, null, 2));
        }

        // Forward to user-provided handler
        if (this.notificationHandler) {
            this.notificationHandler(timestampedNotification);
        }
    }
}

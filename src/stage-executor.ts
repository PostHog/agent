...
export class StageExecutor {
  ...
  async execute(task: Task, stage: WorkflowStage, options: WorkflowExecutionOptions): Promise<WorkflowStageExecutionResult> {
    ...
    switch (agent.agent_type) {
      case 'planning':
        return this.runPlanning(task, cwd);
      case 'execution':
        return this.runExecution(task, cwd, permissionMode);
      case 'review':
      case 'testing':
      default:
  ...
  private async runPlanning(task: Task, cwd: string): Promise<WorkflowStageExecutionResult> {
  ...
  private async runExecution(task: Task, cwd: string, permissionMode: WorkflowExecutionOptions['permissionMode']): Promise<WorkflowStageExecutionResult> {

...
export class GitManager {
  ...
  async createTaskPlanningBranch(taskSlug: string, baseBranch?: string): Promise<string> {
    const branchName = await this.generateUniqueBranchName(taskSlug, 'planning');

    this.logger.debug('Creating unique planning branch', { branchName, taskSlug });
  ...
  async createTaskImplementationBranch(taskSlug: string, planningBranchName?: string): Promise<string> {
    const branchName = await this.generateUniqueBranchName(taskSlug, 'implementation');
  ...
  async commitPlan(taskSlug: string, taskTitle: string): Promise<string> {
    const currentBranch = await this.getCurrentBranch();
    this.logger.debug('Committing plan', { taskSlug, currentBranch });
    ...
    if (!hasChanges) {
      this.logger.info('No plan changes to commit', { taskSlug });
...
for the task. Review the plan before proceeding with implementation.`;
    ...
    this.logger.info('Plan committed', { taskSlug, taskTitle });
    return result;
  }

  async commitImplementation(taskSlug: string, taskTitle: string, planSummary?: string): Promise<string> {
    await this.runGitCommand('add .');

    const hasChanges = await this.hasStagedChanges();
    if (!hasChanges) {
      this.logger.warn('No implementation changes to commit', { taskSlug });
      return 'No changes to commit';
    }
...
Generated by PostHog Agent`;
    ...
    const result = await this.commitChanges(message);
    this.logger.info('Implementation committed', { taskSlug, taskTitle });
    return result;
  }
